...
public interface ProdutoDAO extends JpaRepository<ProdutoEntity, Long> {
	...
	@Transactional
	@Modifying
	@Query("UPDATE ProdutoEntity p SET p.nome =:nome, p.marca =:marca, p.unidadeMedida =:unidadeMedida WHERE p.id =:id")
	void editarProduto(@Param("nome") String nome, @Param("marca") String marca,
			@Param("unidadeMedida") String unidadeMedida, @Param("id") Long id);
